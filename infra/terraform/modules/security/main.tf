...
resource "aws_security_group" "alb_ui" {
  name        = "${var.name_prefix}-alb-ui-sg"
  description = "ALB security group for UI ingress"
  vpc_id      = var.vpc_id
...
resource "aws_iam_role_policy" "task_openai_secret" {
  name = "${var.name_prefix}-openai-secret-access"
...
resource "aws_iam_role_policy" "task_execution_openai_secret" {
  name = "${var.name_prefix}-openai-secret-access-exec"
